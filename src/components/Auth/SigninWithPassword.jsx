...
import FetchAuth, { loginChecker } from '@/auth/django_auth';
...
export default function SigninWithPassword() {
  ...
  async function FetchTokenAPI(e) {
    ...
    try {
      IsLoading(true);
      let access = await FetchAuth(username, password);
      setTimeout(() => {
        ;
        if (loginChecker === 1) {
          IsLoading(false);
  ...
  return (<form>
    <div className="mb-4">
      ...
      <div className="relative">
        <input onChange={e => updateFormInput({ ...formInput, username: e.target.value })}
    ...
    <p className="text-sm text-red-500 ml-2 mb-4">{errorMessage}</p>
    ...
    <div className="mb-4.5">
      {isLoading ?
        (
          ...
          <>
            <button onClick={(e) => FetchTokenAPI(e)}
              className="flex w-full cursor-pointer items-center justify-center gap-2 rounded-2xl bg-[#FF5127] p-4 font-medium text-white transition hover:bg-opacity-90">
              Sign In
            </button>
          </>
        )}
