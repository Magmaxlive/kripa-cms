...
const SolutionsTable = ({ nData }) => {
  ...
  const NavigatePage = (data) => {
    router.push(`/dashboard/solutions/view/update/${data.id}`);
  };

  const FetchDeleteAPI = async (id) => {
    var pr_url = `${baseURL}/api/${id}/DeleteSolutionsView`;
    ...
    let deleteResultStatus = delete_data?.data.result;
