...
const fetchDisclosure = () =>
  axios.get(`${baseURL}/disclosure/`).then(r => {
    const d = r.data;
    return Array.isArray(d) ? d[0] || null : d || null;
  });

const saveDisclosure = (formData) => {
  const id = formData.get("id");
  formData.delete("id");
  return id
    ? axios.patch(`${baseURL}/disclosure/${id}/`, formData, { headers: multipartHeaders })
    : axios.post(`${baseURL}/disclosure/`, formData, { headers: multipartHeaders });
};
...
// ---------- Disclosure statement MODAL ----------
const DisclosureModal = ({ isOpen, onClose, data }) => {
  ...
  const mutation = useMutation({
    mutationFn: saveDisclosure,
    onSuccess: () => {
      queryClient.invalidateQueries(["Disclosure"]);
      toast.success("Disclosure statement updated successfully!");
      onClose();
    },
    onError: () => toast.error("Failed to update Disclosure statement."),
  });
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={isEdit ? "Edit Disclosure statement" : "Setup Disclosure statement"}>
      <div className="flex flex-col gap-5">
        ...
        <RichTextField
          ...
          editorKey={`Disclosure-${data?.id ?? "new"}-${isOpen}`}
  ...
  return (
    <>
      ...
      <div className="bg-white dark:bg-boxdark rounded-lg shadow-sm p-6 border border-stroke dark:border-strokedark">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-semibold">Disclosure Statement</h2>
        ...
        {isLoading ? (
          ...
          <p className="text-gray-400 text-sm">No Disclosure statement Click Setup to get started.</p>
        ) : (
