...
const fetchPrivacyPolicy = () =>
  axios.get(`${baseURL}/privacy-policy/`).then(r => {
    const d = r.data;
    return Array.isArray(d) ? d[0] || null : d || null;
  });

const savePrivacyPolicy = (formData) => {
  const id = formData.get("id");
  formData.delete("id");
  return id
    ? axios.patch(`${baseURL}/privacy-policy/${id}/`, formData, { headers: multipartHeaders })
    : axios.post(`${baseURL}/privacy-policy/`, formData, { headers: multipartHeaders });
};
  ...
  <div className="flex flex-col gap-1.5">
    ...
    <div className={`border rounded-md overflow-visible dark:border-strokedark
      ...
      <CKEditor
        ...
        config={{ toolbar: ["heading","|","bold","italic","underline","|","bulletedList","numberedList","|","blockQuote","link","|","undo","redo" ,"|","insertTable", "mediaEmbed", "|",] }}
      />
    </div>
...
const PrivacyPolicyModal = ({ isOpen, onClose, data }) => {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={isEdit ? "Edit Privacy Policy" : "Setup Privacy Policy"}>
      <div className="flex flex-col gap-5">
        ...
        <RichTextField
          ...
          editorKey={`privacy-${data?.id ?? "new"}-${isOpen}`}
    ...
    <>
      ...
      <div className="bg-white dark:bg-boxdark rounded-lg shadow-sm p-6 border border-stroke dark:border-strokedark">
        ...
        {isLoading ? (
          ...
          <p className="text-gray-400 text-sm">No privacy policy yet. Click Setup to get started.</p>
        ) : (
          <div className="flex flex-col gap-4">
            ...
            <div>
              ...
              <div
                className="ck-content border border-gray-200 dark:border-strokedark rounded-md p-4 text-sm text-gray-700 dark:text-gray-300 max-h-60 overflow-y-auto text-left [&_*]:text-left"
