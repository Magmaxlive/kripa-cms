...
const saveTerms = (formData) => {
  const id = formData.get("id");
  formData.delete("id");
  return id
    ? axios.patch(`${baseURL}/terms/${id}/`, formData, { headers: multipartHeaders })
    : axios.post(`${baseURL}/terms/`, formData, { headers: multipartHeaders });
};
...
const TermsModal = ({ isOpen, onClose, data }) => {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={isEdit ? "Edit Terms & conditions" : "Setup Terms & conditions"}>
      <div className="flex flex-col gap-5">
        ...
        <RichTextField
          ...
          editorKey={`Terms-${data?.id ?? "new"}-${isOpen}`}
  ...
  return (
    <>
      ...
      <div className="bg-white dark:bg-boxdark rounded-lg shadow-sm p-6 border border-stroke dark:border-strokedark">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-semibold">Terms and Condition</h2>
        ...
        {isLoading ? (
          ...
          <p className="text-gray-400 text-sm">No T&C yet. Click Setup to get started.</p>
        ) : (
