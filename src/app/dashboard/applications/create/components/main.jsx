...
const AddPortfolio = ({ xData, pageName }) => {
    const [editorLoaded, setEditorLoaded] = useState(false);
    const [editorData, setEditorData] = useState("");
    const [message, setMessage] = useState("");
    const [isLoading, IsLoading] = useState(false);

    const PostEndPoint = "/CreateApplications";
    const pageUrl = "/applications";
    ...
    const handleInnerFile = (e) => {
        const file = e.target.files[0];
        const validImageTypes = ['image/gif', 'image/jpeg', 'image/png', 'image/webp', 'image/avif'];
        validImageTypes.includes(file?.type) ? setThumbnail(file) ||
            setMessage("") : setThumbnail(null) || setMessage("Only images are accepted");
    };
    ...
    async function FetchPostAPI(e) {
        ...
        if (thumbnailImage?.name) form_data.append('thumbnail', thumbnailImage);
        form_data.append('name', title);
        let meta_data = { "content": editorData, "inner_title": innerTitle };
        form_data.append('meta_data', JSON.stringify(meta_data));
    ...
    async function FetchDetails() {
        if (xData) {
            ...
            ImageName(imageId);
            const imageInnerId = xData?.thumbnail?.split('/').filter(Boolean).pop();
