...
const createService = (formData) => axios.post(`${baseURL}/services/`, formData, { headers: multipartHeaders });
...
const SelectField = ({ label, id, value, onChange, options, error, required, placeholder = "Select…" }) => (
  <div className="flex flex-col gap-1.5">
    ...
    <select
      ...
      className={`border h-11 w-full rounded-md px-3 focus:outline-none focus:ring-2 focus:ring-primary dark:bg-meta-4 bg-white transition
        ${error ? "border-red-400 focus:ring-red-400" : "border-gray-300 dark:border-strokedark"}`}
    >
...
const EMPTY_SERVICE_FORM = {
  ...
  minor_heading: "", main_heading: "", brief_description: "",
};

const formFromService = (data) =>
  data
    ? {
...
const ServiceModal = ({ isOpen, onClose, data, categories }) => {
  const queryClient = useQueryClient();
  const isEdit = !!data;

  const [form, setForm]   = useState(() => formFromService(data));
  ...
  const mutation = useMutation({
    mutationFn: isEdit ? updateService : createService,
    onSuccess: () => {
      queryClient.invalidateQueries(["services"]);
      toast.success(`Service ${isEdit ? "updated" : "created"} successfully!`);
      onClose();
    },
    onError: () => toast.error(`Failed to ${isEdit ? "update" : "create"} service.`),
  });
...
// FAQ MODAL
// ════════════════════════════════════════════════════
const FaqModal = ({ isOpen, onClose, service }) => {
  const queryClient = useQueryClient();
  ...
  const { data: faqs = [], isLoading } = useQuery({
    ...
    queryFn:  () => fetchServiceFaqs(serviceId),
    enabled:  !!serviceId && isOpen,
  });

  const invalidate = () => queryClient.invalidateQueries(["service-faqs", serviceId]);

  const createMutation = useMutation({
    mutationFn: createServiceFaq,
    onSuccess: () => { invalidate(); toast.success("FAQ added!"); setAddingNew(false); },
    onError: () => toast.error("Failed to add FAQ."),
  });
  const updateMutation = useMutation({
    mutationFn: updateServiceFaq,
    onSuccess: () => { invalidate(); toast.success("FAQ updated!"); setEditingId(null); },
    onError: () => toast.error("Failed to update FAQ."),
  });
  const deleteMutation = useMutation({
    mutationFn: deleteServiceFaq,
    ...
    <Modal isOpen={isOpen} onClose={onClose} maxWidth="max-w-2xl"
      title={
        <span className="flex items-center gap-2">
          ...
          FAQs — <span className="text-primary">{service?.title}</span>
        </span>
      }>
      <div className="flex flex-col gap-4">
        {!addingNew && (
...
const ServicesForm = () => {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries(["services"]); toast.success("Service deleted successfully!"); },
  ...
  const filteredServices = React.useMemo(() => {
    ...
    if (activeFilter === "none") return services.filter((s) => !s.category);
    return services.filter((s) => String(s.category) === String(activeFilter));
  }, [services, activeFilter]);

  const handleEdit   = (item) => { setSelectedService(item); setServiceModalOpen(true); };
  const handleAdd    = ()     => { setSelectedService(null); setServiceModalOpen(true); };
  const handleDelete = (id)   => { if (confirm("Are you sure you want to delete this service?")) deleteMutation.mutate(id); };
  const handleFaqs   = (item) => { setFaqService(item); setFaqModalOpen(true); };

  return (
    <>
      <ServiceModal
        key={`${selectedService?.id ?? "new"}-${serviceModalOpen}`}
      ...
      <FaqModal
        isOpen={faqModalOpen}
        onClose={() => { setFaqModalOpen(false); setFaqService(null); }}
        service={faqService}
      />
      ...
      <div className="bg-white dark:bg-boxdark rounded-lg shadow-sm p-6 border border-stroke dark:border-strokedark">
        ...
        {isLoading ? (
          ...
          <p className="text-gray-400 text-sm">{services.length === 0 ? "No services added yet." : "No services in this category."}</p>
        ) : (
            ...
            {filteredServices.map((item) => (
              <div key={item.id}
                ...
                <div className="p-4">
                  ...
                  <button onClick={() => handleFaqs(item)}
